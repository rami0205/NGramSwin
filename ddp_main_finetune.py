...
import os
from ddp_engine_finetune import train_one_epoch
...
def main(gpu, args):
    ...
    for epoch in range(args.total_epochs):
        ...
        if (epoch+1) == args.warm_start_epoch+1:
            model, optimizer, loss_scaler = rebuild_after_warm_start(gpu, model, args)
            if rank==0:
                record_whole_finetune_time(args)
        train_loader.sampler.set_epoch(epoch)
        if not args.test_only:
            train_one_epoch(gpu, rank, model, train_loader, optimizer, loss_scaler, criterion, epoch, (epoch+1)<=args.warm_start_epoch, args)
